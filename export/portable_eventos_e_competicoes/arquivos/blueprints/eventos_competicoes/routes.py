...
def _professores_responsavel_por_academia_para_link(cur, id_assoc):
    """
    Um registro por academia: prioriza o professor cujo usuario_id é gestor da academia
    (usuarios_academias + role gestor_academia); se não houver, o primeiro professor ativo
    da academia (ordenado por nome). Professores sem id_academia entram como grupo próprio
    (um por nome).
    """
    cur.execute(
        """
        SELECT p.id, p.nome, p.id_academia, p.usuario_id, ac.nome AS academia_nome
        FROM professores p
        LEFT JOIN academias ac ON ac.id = p.id_academia
        WHERE p.id_associacao = %s AND p.ativo = 1 AND p.status = 'ativo'
        ORDER BY COALESCE(ac.nome, ''), p.nome
    ...
    for aid_k in sorted(by_acad.keys(), key=lambda x: (x is None, x if x is not None else 0)):
        ...
        if aid_k is not None:
            ...
            prof_uid = prof_uid_por_academia.get(aid_k)
            for uid_try in (gestor_uid, prof_uid):
                if uid_try is None:
                    continue
                for r in lst:
                    u = r.get("usuario_id")
                    if u is not None and int(u) == uid_try:
                        chosen = r
                        break
...
def editar(evento_id):
    ...
    try:
        ...
        professores_para_links = _professores_responsavel_por_academia_para_link(cur, id_assoc)
...
def gestor_incluir_participante_avulso(evento_id):
    """Gestor da associação inclui participante manualmente (mesmos dados do link público)."""
    if session.get("modo_painel") != "associacao":
        flash("Disponível apenas em modo associação.", "danger")
        return redirect(url_for("painel.home"))
    if not (current_user.has_role("gestor_associacao") or current_user.has_role("admin")):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not f or not f.filename:
                ...
                flash("Use arquivo Excel .xlsx.", "danger")
            else:
                try:
        ...
        try:
            ...
            if acao == "iniciar":
                ...
                if campo == "ippon":
                    ...
                    atualizacoes.append(f"pontos_branco_wazaari = {novo_wazaari}")
                    if novo_wazaari >= 2:
                        atualizacoes.append("pontos_branco_ippon = 1")
                    ...
                    atualizacoes.append(f"pontos_branco_shido = {novo_shido}")
                    if novo_shido >= 3:
                    ...
                    atualizacoes.append(f"pontos_branco_yoko = {novo_yoko}")
                    if em_golden_score:
                        atualizacoes.append("status = 'finalizada'")
                        atualizacoes.append("vencedor = 'branco'")
                        atualizacoes.append("tipo_vitoria = 'golden_score'")
                ...
                if campo == "ippon":
                    ...
                    if novo_wazaari > 2:
                        return jsonify({"erro": "Máximo de 2 Waza-ari permitido"}), 400
                    atualizacoes.append(f"pontos_azul_wazaari = {novo_wazaari}")
                    if novo_wazaari >= 2:
                        atualizacoes.append("pontos_azul_ippon = 1")
                    ...
                    if novo_shido > 3:
                        return jsonify({"erro": "Máximo de 3 Shido permitido"}), 400
                    atualizacoes.append(f"pontos_azul_shido = {novo_shido}")
                    if novo_shido >= 3:
                    ...
                    atualizacoes.append(f"pontos_azul_yoko = {novo_yoko}")
                    if em_golden_score:
                        atualizacoes.append("status = 'finalizada'")
                        atualizacoes.append("vencedor = 'azul'")
                        atualizacoes.append("tipo_vitoria = 'golden_score'")
                ...
                if tipo_ponto and cor and tipo_ponto in ["ippon", "wazaari", "shido", "yoko"] and cor in ["branco", "azul"]:
                    ...
                    if valor_atual and valor_atual > 0:
                        ...
                        atualizacoes.append(f"{campo} = {novo_valor}")
                        ...
                        if tipo_ponto == "wazaari":
                            ...
                            if campo_wazaari:
                                ...
                                if wazaari_atual == 2 and novo_valor == 1:
                                    ...
                                    if campo_ippon:
                                        atualizacoes.append(f"{campo_ippon} = 0")
            ...
            if atualizacoes:
                ...
                try:
                    ...
                    current_app.logger.error(f"Erro ao atualizar luta: {db_error}")
                    return jsonify({"erro": f"Erro no banco de dados: {err_str}"}), 500
            else:
